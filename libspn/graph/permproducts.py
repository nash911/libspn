...
class PermProducts(OpNode):
    ...
    def __init__(self, *values, name="PermProducts"):
        ...
        self._input_size = value_sizes[0]
        self._num_inputs = len(values)

        # Calculate numbed of products this node would model.
        if self._num_inputs == 1 or self._input_size == 1:
            self._num_prods = 1
        else:
            self._num_prods = pow(self._input_size, self._num_inputs)
        ...
        # values of the node.
        self._permuted_indices = self.permute_indices(self._num_inputs, self._input_size)
    ...
    def _compute_mpe_path(self, counts, *value_values, add_random=False, use_unweighted=False):
        ...
        def permute_counts(num_inputs, input_size):
            # Function that permutes count values, backward to inputs.
            counts_indices_list = []
            expo = list(range(num_inputs-1, -1, -1))
            ...
            for inp in range(num_inputs):
                block_size = pow(input_size, expo[inp])
                step = pow(input_size, expo[inp]+1)
                for i in range(input_size):
                    start = i * block_size
                    stop = self._num_prods - (block_size * (input_size-i-1))
                    counts_indices_list.append(range_with_blocksize(
                      start, stop, block_size, step))
            return counts_indices_list

        summed_counts = []
        for ind in permute_counts(self._num_inputs, self._input_size):
            summed_counts = summed_counts + [tf.reduce_sum(utils.gather_cols(
              counts, ind), axis=1, keep_dims=True)]

        value_counts = zip([tf.concat_v2(sc, axis=1)
                            for sc in zip(*[iter(summed_counts)] *
                            self._input_size)], value_values)
