...
class TestPerformanceProduct(unittest.TestCase):
    ...
    def test_performance(self):
        ...
        def test(inputs, feed, true_value_output, features,
            ...
            with self.subTest(inputs=inputs, feed=feed,
                ...
                for i in range(0, num_graphs):
                    # Generate 'num_prods' Product nodes, connecting each to
                    # it's permuted inputs
                    P = [spn.Product(*i) for i in inps]

                    # Connect all product nodes to a single root node and
                    # generate weights
                    roots = roots + [spn.Sum(*P)]
                    roots[-1].generate_weights()

                    # List of products of the first Product node of each graph
                ...
                print("\nProduct - %s " % ("Single-graph" if num_graphs == 1
        ...
        path_output = np.ones((batch, 1), dtype=spn.conf.dtype.as_numpy_dtype())
