...
class TestPerformanceSum(unittest.TestCase):
    ...
    def test_performance(self):
        ...
        def test(inputs, ivs, feed, true_value_output, num_sums=1,
            ...
            with self.subTest(inputs=inputs, feed=feed,
                ...
                for i in range(0, num_graphs):
                    s = []
                    # Generate 'num_sums' Sum nodes, connecting each to inputs
                    # and ivs
                    for j in range(0, num_sums):
                        s = s + [spn.Sum(*inputs, ivs=ivs)]
                        if j == 0:
                            w0 = s[-1].generate_weights()
                        else:
                            s[-1].generate_weights()

                    # Connect all sum nodes to a single root node and generate
                    # weights
                    roots = roots + [spn.Sum(*s)]
                    roots[-1].generate_weights()

                    # List of weights of the first Sum node of each graph
                    weights = weights + [w0]
                ...
                print("\nSum (%s) - %s " % (("With IVs" if ivs else "No IVs"),
        ...
        path_output = np.zeros((batch, features),
                               dtype=spn.conf.dtype.as_numpy_dtype())
        path_output[:, 0] = 1.0
