...
class TestPerformanceProducts(unittest.TestCase):

    def tearDown(self):
        tf.reset_default_graph()

    def test_performance(self):
        """Testing performance of Products op"""

        def test(inputs, feed, true_value_output, features,
                 num_inputs, num_graphs=1, true_path_output=None):
            with self.subTest(inputs=inputs, feed=feed,
                ...
                for indices in permuted_inds_list_of_list:
                    inps = inps + [tuple(i) for i in zip(inputs, indices)]
                ...
                for i in range(0, num_graphs):
                    # Create a single Products node, modeling 'num_prods' products
                    # within, connecting it to inputs
                    P = spn.Products(*inps, num_prods=num_prods)

                    # Connect the Products nodes to a single root node and
                    # generate weights
                    roots = roots + [spn.Sum(P)]
                    roots[-1].generate_weights()

                    # List of Products node of each graph
                    products = products + [P]
        ...
        test(inputs, {inputs[0]: inputs_feed}, value_output,
             features, num_inputs, num_graphs, path_output)
